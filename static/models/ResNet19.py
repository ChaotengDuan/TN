...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, stride=1, downsample=None, method='base'):
        ...
        if method == 'TN':
            ...
            self.conv1 = BNTTLayer(in_ch, out_ch, 3, stride, 1)
        self.sn1 = LIFSpike()
        ...
        if method == 'TN':
            ...
            self.conv2 = BNTTLayer(out_ch, out_ch, 3, 1, 1)
        self.sn2 = LIFSpike()
...
class ResNet(nn.Module):
    def __init__(self, block=BasicBlock, layers=[3, 3, 2], method='base'):
        ...
        elif  self.method == 'BNTT':
            self.conv1  = BNTTLayer(3, self.in_ch, 3, 1, 1)
    ...
    def make_layer(self, block, in_ch, block_num, stride=1):
        ...
        if stride != 1 or self.in_ch != in_ch * block.expansion: 
            if self.method == 'TN':
                ...
                downsample = BNTTLayer(self.in_ch, in_ch * block.expansion, 1, stride, 0)
            elif  self.method == 'base':
